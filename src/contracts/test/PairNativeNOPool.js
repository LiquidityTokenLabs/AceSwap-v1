...
describe('プール別テスト(SingleNativeOPool)', function () {
  before('コントラクトデプロイ&ミント', async () => {
    ...
    FactoryPairNativeNOPool = await ethers.getContractFactory('FactoryPairNativeNOPool')
    factoryPairNativeNOPool = await FactoryPairNativeNOPool.deploy(router.address, protocolFeeRatio)
  ...
  describe('Routerの初期設定', () => {
    ...
    it('SET:Factoryの登録', async () => {
      await router.setFactoryApprove(factoryPairNativeNOPool.address, true)
      expect(await router.getIsFactoryApprove(factoryPairNativeNOPool.address)).to.true
    })
  })

  describe('ファクトリーによるプールの作成', () => {
    it('MAIN:Factory721によるプールの作成', async function () {
      pool721Address = await factoryPairNativeNOPool
        .connect(staker)
        .createPool(sampleNFT.address, bondingCurve.address, spotPrice, delta, spread)
      pool721pend = await (await pool721Address).wait()
      await expect(pool721Address).to.emit(factoryPairNativeNOPool, 'CreatePool')
    })
    ...
    it('GET:プールのインスタンス化', async function () {
      ...
      pairNativeOPool = await new ethers.Contract(
        ...
        factoryPairNativeNOPool.signer
      )
    })
  })
  ...
  describe('プールを作成したユーザー以外はFTをステーキングできない', () => {
    it('他のユーザーがステーキングしようとするとRevertする', async () => {
      ...
      await expect(
        router.connect(staker2).stake(pairNativeOPool.address, ['12'], {
          value: ethers.utils.parseEther('0.0072')
        })
  ...
  describe('2個目のプールを作成', () => {
    it('Factory721によるプールの作成', async function () {
      pool721Address2 = await factoryPairNativeNOPool
        .connect(staker)
        .createPool(sampleNFT.address, bondingCurve.address, spotPrice, delta, spread)
      pool721pend2 = await (await pool721Address2).wait()
      await expect(pool721Address2).to.emit(factoryPairNativeNOPool, 'CreatePool')
    })
    ...
    it('プールのインスタンス化', async function () {
      const Pool721 = await ethers.getContractFactory('SingleNativeOPool')
      pairNativeNOPool2 = await new ethers.Contract(
        pool721tast2,
        Pool721.interface.format(),
        factoryPairNativeNOPool.signer
      )
  ...
  describe('ペアステーキング→2プールからFTからNFTにスワップ→2プールからNFTからFTにスワップ→流動性解除', () => {
    ...
    it('FTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(staker).approve(pairNativeNOPool2.address, 17)
      expect(
        await router.connect(staker).stake(pairNativeNOPool2.address, ['17'], {
          value: ethers.utils.parseEther('0.0072')
        })
      ).to.emit(router, 'Stake')
    })
    ...
    it('2プールからFT->NFT(batchSwapFTforNFT)', async function () {
      await router
        .connect(swapperFTforNFT)
        .batchSwapFTforNFT(
          [pairNativeOPool.address, pairNativeNOPool2.address],
          [[['13']], [['17']]],
          supporter1.address,
          {
    ...
    it('2プールからNFT->FT(batchSwapNFTforFT)', async function () {
      ...
      await sampleNFT.connect(swapperNFTforFT).approve(pairNativeNOPool2.address, 16)
