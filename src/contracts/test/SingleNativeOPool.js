...
describe('プール別テスト(SingleNativeOPool)', function () {
  before('コントラクトデプロイ&ミント', async () => {
    ...
    FactorySingleNativeOPool = await ethers.getContractFactory('FactorySingleNativeOPool')
    factorySingleNativeOPool = await FactorySingleNativeOPool.deploy(router.address, protocolFeeRatio)
  ...
  describe('Routerの初期設定', () => {
    it('SET:ボンディングカーブの登録', async () => {
      await expect(router.setBondingCurveApprove(bondingCurve.address, true)).to.emit(router, 'UpdateBondingCurve')
    ...
    it('SET:コレクションの登録', async () => {
      await expect(router.setCollectionApprove(sampleNFT.address, true)).to.emit(router, 'UpdateCollection')
      expect(await router.getIsCollectionApprove(sampleNFT.address)).to.true
    })
    it('SET:Factoryの登録', async () => {
      await expect(router.setFactoryApprove(factorySingleNativeOPool.address, true)).to.emit(router, 'UpdateFactory')
      expect(await router.getIsFactoryApprove(factorySingleNativeOPool.address)).to.true
    })
  })

  describe('ファクトリーによるプールの作成', () => {
    it('MAIN:Factory721によるプールの作成', async function () {
      pool721Address = await factorySingleNativeOPool.createPool(
        sampleNFT.address,
      ...
      await expect(pool721Address).to.emit(factorySingleNativeOPool, 'CreatePool')
    })
    ...
    it('GET:プールのインスタンス化', async function () {
      ...
      singleNativeOPool = await new ethers.Contract(
        pool721tast,
        Pool721.interface.format(),
        factorySingleNativeOPool.signer
      )
    })
  })
  ...
  describe('NFTをステーキング → 流動性解除', () => {
    ...
    it('MAIN:NFTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(stakerNFT).approve(singleNativeOPool.address, 1)
      expect(await router.connect(stakerNFT).stakeNFT(singleNativeOPool.address, ['1'])).to.emit(router, 'StakeNFT')
    })
    it('GET:NFTが元の所有者であるstakerAになる', async () => {
      expect(await sampleNFT.ownerOf(1)).to.equal(singleNativeOPool.address)
    })
    ...
    it('GET:stakeNFTPriceは需要に合わせて増加する', async () => {
      expect(await singleNativeOPool.stakeNFTprice()).to.equal(ethers.utils.parseEther('0.011'))
    })
    it('MAIN:NFTの流動性を解除するとEventが発行される', async () => {
      expect(await router.connect(stakerNFT).withdrawNFT(singleNativeOPool.address, ['1'])).to.emit(
        router,
        'WithdrawNFT'
      )
    })
    it('GET:ステーキングが解除されたのでstakeNFTpriceが下がる', async () => {
      expect(await singleNativeOPool.stakeNFTprice()).to.equal(ethers.utils.parseEther('0.01'))
    })
    it('GET:NFTが元の所有者であるstakerAになる', async () => {
      expect(await sampleNFT.ownerOf(1)).to.equal(stakerNFT.address)
    })
  ...
  describe('NFTをステーキング → FTでスワップ → 流動性解除', () => {
    ...
    it('GET:stakeFTprice', async () => {
      expect(await singleNativeOPool.stakeFTprice()).to.equal(ethers.utils.parseEther('0.01'))
    })
    it('GET:[状態確認]poolInfoの状態は想定通りである', async () => {
      poolInfo = await singleNativeOPool.getPoolInfo()
      expect(poolInfo.spotPrice).to.equal(ethers.utils.parseEther('0.01'))
    ...
    it('MAIN:FTからNFTにスワップをするとEventが発行される', async () => {
      let beforBalance = await ethers.provider.getBalance(singleNativeOPool.address)
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(singleNativeOPool.address, ['1'], supporter1.address, {
          value: ethers.utils.parseEther('0.01')
        })
      ).to.emit(router, 'SwapFTforNFT')
      let afterBalance = await ethers.provider.getBalance(singleNativeOPool.address)
    ...
    it('MAIN:NFTの流動性を解除するとEventが発行される', async () => {
      ...
      expect(await router.connect(stakerNFT).withdrawNFT(singleNativeOPool.address, [])).to.emit(router, 'WithdrawNFT')
  ...
  describe('NFTをステーキング → FTからNFTにスワップ → NFTからFTにスワップ → 流動性解除', () => {
    ...
    it('GET:stakeFTprice', async () => {
      expect(await singleNativeOPool.stakeFTprice()).to.equal(ethers.utils.parseEther('0.011'))
    })
    ...
    it('MAIN:NFTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(stakerNFT).approve(singleNativeOPool.address, 2)
      expect(await router.connect(stakerNFT).stakeNFT(singleNativeOPool.address, ['2'])).to.emit(router, 'StakeNFT')
    })
    it('GET:NFTの所有者がswapperFTforNFTになる', async () => {
      expect(await sampleNFT.ownerOf(2)).to.equal(singleNativeOPool.address)
    })
    ...
    it('MAIN:FTからNFTにスワップをするとEventが発行される', async () => {
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(singleNativeOPool.address, ['2'], supporter1.address, {
    ...
    it('MAIN:NFTからFTにスワップするとEventが発行される', async () => {
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 3)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(singleNativeOPool.address, ['3'], ethers.utils.parseEther('0.0088'), supporter1.address)
    ...
    it('MAIN:NFTの流動性を解除すると流動性報酬が入り残高が増える', async () => {
      let aaa = await ethers.provider.getBalance(singleNativeOPool.address)
      aaa
      let beforBalance = await ethers.provider.getBalance(stakerNFT.address)
      expect(await router.connect(stakerNFT).withdrawNFT(singleNativeOPool.address, ['3'])).to.emit(
        router,
        'WithdrawNFT'
      )
      let afterBalance = await ethers.provider.getBalance(stakerNFT.address)
  ...
  describe('NFTをステーキング → FTからNFTにスワップ → NFTからFTにスワップ → 流動性解除', () => {
    ...
    it('NFTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(stakerNFT).approve(singleNativeOPool.address, 4)
      expect(await router.connect(stakerNFT).stakeNFT(singleNativeOPool.address, ['4'])).to.emit(router, 'StakeNFT')
    })
    it('GET:[状態確認]buyNumが1増える', async () => {
      poolInfo = await singleNativeOPool.getPoolInfo()
      expect(poolInfo.spotPrice).to.equal(ethers.utils.parseEther('0.011'))
    ...
    it('MAIN:FTからNFTにスワップをするとEventが発行される', async () => {
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(singleNativeOPool.address, ['4'], supporter1.address, {
          value: ethers.utils.parseEther('0.011')
        })
      ).to.emit(router, 'SwapFTforNFT')
    })
    ...
    it('MAIN:NFTからFTにスワップするとEventが発行される', async () => {
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 5)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(singleNativeOPool.address, ['5'], ethers.utils.parseEther('0.0088'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    it('GET:[状態確認]sellNumが1減って、buyNumが1増える、spotPriceが減る', async () => {
      poolInfo = await singleNativeOPool.getPoolInfo()
    ...
    it('MAIN:NFTの流動性を解除すると流動性報酬が入り残高が増える', async () => {
      let beforBalance = await ethers.provider.getBalance(stakerNFT.address)
      expect(await router.connect(stakerNFT).withdrawNFT(singleNativeOPool.address, ['5'])).to.emit(
        router,
        'WithdrawNFT'
      )
      let afterBalance = await ethers.provider.getBalance(stakerNFT.address)
  ...
  describe('2人がNFTをステーキング → FTからNFTにスワップ → NFTからFTにスワップ → 流動性解除', () => {
    it('NFTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(stakerNFT).approve(singleNativeOPool.address, 5)
      expect(await router.connect(stakerNFT).stakeNFT(singleNativeOPool.address, ['5'])).to.emit(router, 'StakeNFT')
    })
    it('GET:[状態確認]userInfo', async () => {
      userInfo = await singleNativeOPool.getUserInfo(stakerNFT.address)
    ...
    it('NFTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(stakerNFT2).approve(singleNativeOPool.address, 6)
      expect(await router.connect(stakerNFT2).stakeNFT(singleNativeOPool.address, ['6'])).to.emit(router, 'StakeNFT')
    })
    it('GET:[状態確認]userInfo', async () => {
      userInfo = await singleNativeOPool.getUserInfo(stakerNFT2.address)
      expect(userInfo.initBuyNum).to.equal(1)
      expect(userInfo.initSellNum).to.equal(0)
      expect(userInfo.initSellAmount).to.equal(ethers.utils.parseEther('0'))
    ...
    it('MAIN:FTからNFTにスワップをするとEventが発行される', async () => {
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(singleNativeOPool.address, ['5', '6'], supporter1.address, {
          value: ethers.utils.parseEther('0.023')
        })
      ).to.emit(router, 'SwapFTforNFT')
    })
    it('MAIN:NFTからFTにスワップをするとEventが発行される', async () => {
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 7)
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 8)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(singleNativeOPool.address, ['7', '8'], ethers.utils.parseEther('0.0184'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    //0.00176 -> 0.00158
    it('MAIN:NFTの流動性を解除すると報酬が入り残高が増える', async () => {
      let beforBalance = await ethers.provider.getBalance(stakerNFT.address)
      expect(await router.connect(stakerNFT).withdrawNFT(singleNativeOPool.address, ['7'])).to.emit(
        router,
        'WithdrawNFT'
      )
      let afterBalance = await ethers.provider.getBalance(stakerNFT.address)
    ...
    it('MAIN:NFTの流動性を解除すると報酬が入り残高が増える', async () => {
      ...
      expect(await router.connect(stakerNFT2).withdrawNFT(singleNativeOPool.address, ['8'])).to.emit(
        router,
        'WithdrawNFT'
      )
  ...
  describe('FTをステーキング → 流動性解除', () => {
    ...
    it('MAIN:FTをステーキングするとEventが発行される', async () => {
      expect(
        await router.connect(stakerFT).stakeFT(singleNativeOPool.address, 1, {
    ...
    it('GET:[状態確認]userInfo', async () => {
      userInfo = await singleNativeOPool.getUserInfo(stakerFT.address)
    ...
    it('MAIN:FTの流動性を解除するとEventが発行される', async () => {
      ...
      await expect(router.connect(stakerFT).withdrawFT(singleNativeOPool.address, 1, [])).to.emit(router, 'WithdrawFT')
  ...
  describe('FTをステーキング → NFTからFTにスワップ  → 流動性解除', () => {
    ...
    it('MAIN:FTからNFTにスワップするとEventが発行される', async () => {
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 9)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(singleNativeOPool.address, ['9'], ethers.utils.parseEther('0.0072'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    it('GET:NFTの所有者がswapperFTforNFTになる', async () => {
      expect(await sampleNFT.ownerOf(9)).to.equal(singleNativeOPool.address)
    })
    it('MAIN:NFTの流動性を解除するとEventが発行される', async () => {
      await expect(router.connect(stakerFT).withdrawFT(singleNativeOPool.address, 0, ['9'])).to.emit(
        router,
        'WithdrawFT'
      )
    })
  ...
  describe('FTをステーキング → NFTからFTにスワップ → FTからNFTにスワップ → 流動性解除', () => {
    ...
    it('MAIN:FTからNFTにスワップするとEventが発行される', async () => {
      await sampleNFT.connect(swapperFTforNFT).approve(singleNativeOPool.address, 10)
      expect(
        await router
          .connect(swapperFTforNFT)
          .swapNFTforFT(singleNativeOPool.address, ['10'], ethers.utils.parseEther('0.0072'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    it('MAIN:FTからNFTにスワップするとEventが発行される', async () => {
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(singleNativeOPool.address, ['10'], supporter1.address, {
  ...
  describe('2人がFTをステーキング → FTからNFTにスワップ → NFTからFTにスワップ → 流動性解除', () => {
    ...
    it('FTをステーキングするとEventが発行される', async () => {
      expect(
        await router.connect(stakerFT2).stakeFT(singleNativeOPool.address, 1, {
          value: ethers.utils.parseEther('0.0064')
        })
      ).to.emit(router, 'StakeFT')
    })
    it('NFTからFTにスワップするとEventが発行される', async () => {
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 11)
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 12)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(singleNativeOPool.address, ['11', '12'], ethers.utils.parseEther('0.0136'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    it('FTからNFTにスワップするとEventが発行される', async () => {
      expect(
        await router
          .connect(swapperFTforNFT)
          .swapFTforNFT(singleNativeOPool.address, ['11', '12'], supporter1.address, {
    ...
    it('NFTの流動性を解除するとEventが発行される', async () => {
      ...
      console.log('StakerFTが受け取った量:' + afterBalance.sub(beforBalance).toString())
    })
    it('NFTの流動性を解除するとEventが発行される', async () => {
      let beforBalance = await ethers.provider.getBalance(stakerFT2.address)
      await expect(router.connect(stakerFT2).withdrawFT(singleNativeOPool.address, 1, [])).to.emit(router, 'WithdrawFT')
      ...
      console.log('StakerFT2が受け取った量:' + afterBalance.sub(beforBalance).toString())
    })
    it('サポーターFeeが適切にもらえる(routerの残高の確認)', async () => {
      let Balance = await ethers.provider.getBalance(router.address)
      console.log(Balance + ':Routerの残高')
  ...
  describe('NFTをステーキング → FTをステーキング → NFTからFTにスワップ → FTからNFTにスワップ → 流動性解除', () => {
    it('NFTをステーキングするとEventが発行される', async () => {
      let Balance = await ethers.provider.getBalance(router.address)
      console.log(Balance + ':Routerの残高')
      await sampleNFT.connect(stakerNFT).approve(singleNativeOPool.address, 13)
      expect(await router.connect(stakerNFT).stakeNFT(singleNativeOPool.address, ['13'])).to.emit(router, 'StakeNFT')
    })
    it('FTをステーキングするとEventが発行される', async () => {
      let Balance = await ethers.provider.getBalance(router.address)
      console.log(Balance + ':Routerの残高')
    ...
    it('FTからNFTにスワップするとEventが発行される', async () => {
      ...
      console.log(Balance + ':Routerの残高')
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(singleNativeOPool.address, ['13'], supporter1.address, {
    ...
    it('FTからNFTにスワップするとEventが発行される', async () => {
      let Balance = await ethers.provider.getBalance(router.address)
      console.log(Balance + ':Routerの残高')
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 14)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(singleNativeOPool.address, ['14'], ethers.utils.parseEther('0.0080'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    it('NFTの流動性を解除するとEventが発行される', async () => {
      let Balance = await ethers.provider.getBalance(router.address)
      console.log(Balance + ':Routerの残高')
      let beforBalance = await ethers.provider.getBalance(stakerNFT.address)
      await expect(router.connect(stakerNFT).withdrawNFT(singleNativeOPool.address, ['14'])).to.emit(
        router,
        'WithdrawNFT'
      )
      let afterBalance = await ethers.provider.getBalance(stakerNFT.address)
  ...
  describe('2個目のプールを作成', () => {
    it('Factory721によるプールの作成', async function () {
      pool721Address2 = await factorySingleNativeOPool.createPool(
        sampleNFT.address,
        bondingCurve.address,
        spotPrice,
        delta,
      ...
      await expect(pool721Address2).to.emit(factorySingleNativeOPool, 'CreatePool')
    })
    ...
    it('プールのインスタンス化', async function () {
      const Pool721 = await ethers.getContractFactory('SingleNativeOPool')
      singleNativeOPool2 = await new ethers.Contract(
        pool721tast2,
        Pool721.interface.format(),
        factorySingleNativeOPool.signer
      )
  ...
  describe('2個目のプールを作成', () => {
    it('2プールに2つのNFTをステーク', async function () {
      await sampleNFT.connect(stakerNFT).approve(singleNativeOPool.address, 15)
      await sampleNFT.connect(stakerNFT).approve(singleNativeOPool2.address, 16)
      await router
        .connect(stakerNFT)
        .batchStakeNFT([singleNativeOPool.address, singleNativeOPool2.address], [[['15']], [['16']]])
    })
    it('NFTはプールが持っている', async function () {
      expect(await sampleNFT.ownerOf(15)).to.equal(singleNativeOPool.address)
      expect(await sampleNFT.ownerOf(16)).to.equal(singleNativeOPool2.address)
    })
    it('2プールに2つのFTをステーク', async function () {
      await router.connect(stakerFT).batchStakeFT([singleNativeOPool.address, singleNativeOPool2.address], [1, 1], {
    ...
    it('2プールからFT->NFT', async function () {
      await router
        .connect(swapperFTforNFT)
        .batchSwapFTforNFT(
          [singleNativeOPool.address, singleNativeOPool2.address],
          [[['15']], [['16']]],
          supporter1.address,
          {
    ...
    it('2プールからNFT->FT', async function () {
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool.address, 17)
      await sampleNFT.connect(swapperNFTforFT).approve(singleNativeOPool2.address, 18)
