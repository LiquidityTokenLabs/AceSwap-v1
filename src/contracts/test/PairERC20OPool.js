...
describe('プール別テスト(PairERC20OPool)', function () {
  before('コントラクトデプロイ&ミント', async () => {
    ...
    FactoryPairERC20OPool = await ethers.getContractFactory('FactoryPairERC20OPool')
    factoryPairERC20OPool = await FactoryPairERC20OPool.deploy(router.address, protocolFeeRatio)
  ...
  describe('Routerの初期設定', () => {
    ...
    it('SET:Factoryの登録', async () => {
      await router.setFactoryApprove(factoryPairERC20OPool.address, true)
      expect(await router.getIsFactoryApprove(factoryPairERC20OPool.address)).to.true
    })
  ...
  describe('ファクトリーによるプールの作成', () => {
    it('MAIN:Factory721によるプールの作成', async function () {
      pool721Address = await factoryPairERC20OPool.createPool(
        sampleNFT.address,
      ...
      await expect(pool721Address).to.emit(factoryPairERC20OPool, 'CreatePool')
    })
    ...
    it('GET:プールのインスタンス化', async function () {
      ...
      pairERC20OPool = await new ethers.Contract(pool721tast, Pool721.interface.format(), factoryPairERC20OPool.signer)
    })
  })
  ...
  describe('ペアでステーキング → FTでスワップ → 流動性解除', () => {
    ...
    it('GET:totalFTpointとtotalNFTpoint', async () => {
      totalFTpoint = await pairERC20OPool.totalFTpoint()
      totalNFTpoint = await pairERC20OPool.totalNFTpoint()
  ...
  describe('2人がペアでステーキング → FTからNFTにスワップ → NFTからFTにスワップ → 流動性解除', () => {
    ...
    it('NFTをステーキングするとEventが発行される', async () => {
      ...
      await expect(router.connect(staker).stake(pairERC20OPool.address, ['5'])).to.emit(router, 'Stake')
    })
    ...
    it('NFTをステーキングするとEventが発行される', async () => {
      ...
      await sampleFT.connect(staker2).approve(pairERC20OPool.address, ethers.utils.parseEther('0.0072'))
      expect(await router.connect(staker2).stake(pairERC20OPool.address, ['6'])).to.emit(router, 'StakeNFT')
    })
    it('GET:[状態確認]userInfo', async () => {
    ...
    it('MAIN:FTからNFTにスワップをするとEventが発行される', async () => {
      await sampleFT.connect(swapperFTforNFT).approve(pairERC20OPool.address, ethers.utils.parseEther('0.023'))
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(pairERC20OPool.address, ['5', '6'], supporter1.address)
      ).to.emit(router, 'SwapFTforNFT')
    })
    it('MAIN:NFTからFTにスワップをするとEventが発行される', async () => {
      await sampleNFT.connect(swapperNFTforFT).approve(pairERC20OPool.address, 7)
      await sampleNFT.connect(swapperNFTforFT).approve(pairERC20OPool.address, 8)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(pairERC20OPool.address, ['7', '8'], ethers.utils.parseEther('0.0184'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    //0.00176 -> 0.00158
    it('MAIN:NFTの流動性を解除すると報酬が入り残高が増える', async () => {
      let beforBalance = await sampleFT.connect(owner).balanceOf(staker.address)
      await expect(router.connect(staker).withdraw(pairERC20OPool.address, 1, ['7'])).to.emit(router, 'Withdraw')
      let afterBalance = await sampleFT.connect(owner).balanceOf(staker.address)
      expect(afterBalance.sub(beforBalance)).to.above(ethers.utils.parseEther('0.00157'))
    })
    //0.00192 -> 0.00172
    it('MAIN:NFTの流動性を解除すると報酬が入り残高が増える', async () => {
      let beforBalance = await sampleFT.connect(owner).balanceOf(staker2.address)

      await expect(router.connect(staker2).withdraw(pairERC20OPool.address, 1, ['8'])).to.emit(router, 'Withdraw')
      let afterBalance = await sampleFT.connect(owner).balanceOf(staker2.address)
      expect(afterBalance.sub(beforBalance)).to.above(ethers.utils.parseEther('0.00172'))
      let Balance = await sampleFT.connect(owner).balanceOf(pairERC20OPool.address)
  ...
  describe('2人がペアでステーキング → FTからNFTにスワップ → NFTからFTにスワップ → 流動性解除', () => {
    ...
    it('FTをステーキングするとEventが発行される', async () => {
      ...
      await sampleFT.connect(staker).approve(pairERC20OPool.address, ethers.utils.parseEther('0.0072'))
      expect(await router.connect(staker).stake(pairERC20OPool.address, ['11'])).to.emit(router, 'Stake')
    })
    it('FTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(staker2).approve(pairERC20OPool.address, 12)
      await sampleFT.connect(staker2).approve(pairERC20OPool.address, ethers.utils.parseEther('0.0064'))
      expect(await router.connect(staker2).stake(pairERC20OPool.address, ['12'])).to.emit(router, 'Stake')
    })
    it('NFTからFTにスワップするとEventが発行される', async () => {
      await sampleNFT.connect(swapperNFTforFT).approve(pairERC20OPool.address, 13)
      await sampleNFT.connect(swapperNFTforFT).approve(pairERC20OPool.address, 14)
      expect(
        await router
          .connect(swapperNFTforFT)
          .swapNFTforFT(pairERC20OPool.address, ['13', '14'], ethers.utils.parseEther('0.0136'), supporter1.address)
      ).to.emit(router, 'SwapNFTforFT')
    })
    it('FTからNFTにスワップするとEventが発行される', async () => {
      await sampleFT.connect(swapperFTforNFT).approve(pairERC20OPool.address, ethers.utils.parseEther('0.017'))
      expect(
        await router.connect(swapperFTforNFT).swapFTforNFT(pairERC20OPool.address, ['11', '12'], supporter1.address)
      ).to.emit(router, 'SwapFTforNFT')
    })
    it('NFTの流動性を解除するとEventが発行される', async () => {
      let beforBalance = await sampleFT.connect(owner).balanceOf(staker.address)
      await expect(router.connect(staker).withdraw(pairERC20OPool.address, 1, ['13'])).to.emit(router, 'Withdraw')
      let afterBalance = await sampleFT.connect(owner).balanceOf(staker.address)
    ...
    it('NFTの流動性を解除するとEventが発行される', async () => {
      let beforBalance = await sampleFT.connect(owner).balanceOf(staker2.address)
      await expect(router.connect(staker2).withdraw(pairERC20OPool.address, 1, ['14'])).to.emit(router, 'Withdraw')
      let afterBalance = await sampleFT.connect(owner).balanceOf(staker2.address)
  ...
  describe('2個目のプールを作成', () => {
    it('Factory721によるプールの作成', async function () {
      pool721Address2 = await factoryPairERC20OPool.createPool(
        sampleNFT.address,
        bondingCurve.address,
        spotPrice,
        delta,
      ...
      await expect(pool721Address2).to.emit(factoryPairERC20OPool, 'CreatePool')
    })
    ...
    it('プールのインスタンス化', async function () {
      const Pool721 = await ethers.getContractFactory('SingleNativeOPool')
      pairERC20OPool2 = await new ethers.Contract(
        pool721tast2,
        Pool721.interface.format(),
        factoryPairERC20OPool.signer
      )
    })
  })
  ...
  describe('ペアステーキング→2プールからFTからNFTにスワップ→2プールからNFTからFTにスワップ→流動性解除', () => {
    ...
    it('FTをステーキングするとEventが発行される', async () => {
      await sampleNFT.connect(staker).approve(pairERC20OPool2.address, 17)
      await sampleFT.connect(staker).approve(pairERC20OPool2.address, ethers.utils.parseEther('0.0072'))
      expect(await router.connect(staker).stake(pairERC20OPool2.address, ['17'])).to.emit(router, 'Stake')
    })
    ...
    it('2プールからFT->NFT(batchSwapFTforNFT)', async function () {
      ...
      await sampleFT.connect(swapperFTforNFT).approve(pairERC20OPool2.address, ethers.utils.parseEther('0.012'))
      await router
        .connect(swapperFTforNFT)
        .batchSwapFTforNFT([pairERC20OPool.address, pairERC20OPool2.address], [[['13']], [['17']]], supporter1.address)
    })
    it('2プールからNFT->FT(batchSwapNFTforFT)', async function () {
      ...
      await sampleNFT.connect(swapperNFTforFT).approve(pairERC20OPool2.address, 16)
      let beforBalance = await sampleFT.connect(owner).balanceOf(router.address)
      await router
        .connect(swapperNFTforFT)
        .batchSwapNFTforFT(
          [pairERC20OPool.address, pairERC20OPool2.address],
          [[['15']], [['16']]],
